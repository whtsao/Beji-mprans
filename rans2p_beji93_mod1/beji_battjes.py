...
vertices = [( 0.00, 0.00), # 0
            ...
            (28.95, 0.30), # 7
	    (tank_dim[0], 0.30), # 8
	    (tank_dim[0], tank_dim[1]), # 9
	    (28.95, tank_dim[1]), # 10
	    ( 0.00, tank_dim[1])] # 11

vertexFlags = [boundaryTags['y-'], # 0
               boundaryTags['y-'], # 1
               boundaryTags['y-'], # 2
               boundaryTags['y-'], # 3
               boundaryTags['y-'], # 4
...
segments = [(i, (i+1)%nVertices) for i in range(nVertices)]+[(7,10)]
...
dragAlpha = 1.e+6
xmcenter = [0.5*(28.95+tank_dim[0]), 0.5*(0.75+0.3)]  # Set the center for the x absorption zone
...
column_gauge_locations = ((( 6.0,          0.0, 0.0), ( 6.0, tank_dim[1], 0.0)),
                          ...
                          ((tank_dim[0]-0.1, 0.3, 0.0), (tank_dim[0]-0.1, tank_dim[1], 0.0)))
